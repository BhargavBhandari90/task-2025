...
registerBlockType('task-2025/slide-item', {
    ...
    attributes: {
        ...
        description: { type: 'string', source: 'html', selector: '.slide-item__desc' }
    },
    edit: (props) => {
            ...
            <div {...blockProps}>
                <div className="slide-item__icon">
                    <MediaUpload
                        ...
                        allowedTypes={[ 'image' ]}
                        render={({ open }) => (
                            <Button onClick={open} variant="secondary">{ attributes.iconUrl ? 'Change Icon' : 'Upload Icon' }</Button>
                        )}
                    />
    ...
    save: ({ attributes }) => {
        ...
        return (
            <div {...blockProps}>
                {attributes.iconUrl && <img className="slide-item__icon" src={attributes.iconUrl} alt="" />}
                <RichText.Content tagName="h4" value={attributes.title} />
                <div className="slide-item__desc" dangerouslySetInnerHTML={{ __html: attributes.description || '' }} />
            </div>
        );
    }
});
...
registerBlockType('task-2025/advanced-slider', {
    ...
    edit: ( props ) => {
        ...
        const innerCount = useSelect( ( select ) => {
            const block = select( 'core/block-editor' ).getBlock( clientId );
            if ( !block ) return 0;
            return (block.innerBlocks || []).filter( ( b ) => b.name === 'task-2025/slide-item' ).length;
        }, [ clientId ] );
        ...
        return (
            <div {...blockProps} className={ hasNav ? 'slider-wrapper has-more-than-5' : 'slider-wrapper' }>
                <div className="slider-grid">
                    <InnerBlocks
                        allowedBlocks={[ 'task-2025/slide-item' ]}
                        template={[ [ 'task-2025/slide-item' ], [ 'task-2025/slide-item' ] ]}
    ...
    save: ( { attributes } ) => {
        ...
        const blockProps = useBlockProps.save({ className: hasNav ? 'slider-wrapper has-more-than-5' : 'slider-wrapper' });
        return (
            <div {...blockProps}>
                <div className="slider-grid">
                    <InnerBlocks.Content />
